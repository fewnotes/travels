(function (global) {
  ...
  function _findOneDrivePhotos(regionId, token, cb) {
    ...
    var path = "/me/drive/root:/" + DRIVE_ROOT_NAME + "/" + PAGE_KEY + "/" + regionId + ":/children?$select=id,name,file,photo";
    _graphGet(path, token, function (err, data) {
      ...
      var mapped = images.map(function (f) {
        ...
        return {
          id: f.id,
          name: f.name,
          downloadUrl: "https://graph.microsoft.com/v1.0/me/drive/items/" + f.id + "/content",
